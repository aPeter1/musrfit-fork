...
int PMusrWiz::writeMsrFileSingleHisto()
{
  ...
  for (int i=0; i<fMsrData->getNoOfParam(); i++) {
    ...
    fout << left << param.getPosErr();
    if (!param.getBoundLow().isEmpty())
      fout << left << param.getBoundLow();
  ...
  for (int i=0; i<noOfDetec; i++) {
    ...
    for (int j=0; j<fMsrData->getNoOfMap(); j++) {
      ...
      fout << qSetFieldWidth(9);
      fout << right << fMsrData->getNoOfParam() + 1 + j + (fMsrData->getNoOfMap()+2)*i;
      fout << qSetFieldWidth(0) << "  ";
      fout << qSetFieldWidth(11);
      ...
      if (map.getName().startsWith("ph", Qt::CaseInsensitive) ||
        ...
        fout << left << detector->getRelGeomPhase();
        ...
        fout << left << map.getValue();
        fout << left << map.getStep();
        fout << left << map.getPosErr();
        if (map.getBoundLow() != "")
          fout << left << map.getBoundLow();
        if (map.getBoundHigh() != "")
          fout << left << map.getBoundHigh();
      }
      fout << qSetFieldWidth(0);
      fout << endl;
#else
  ...
  for (int i=0; i<noOfDetec; i++) {
    ...
    if (fMsrData->getT0Tag() == T0_ENTER_WIZ) {
      fout << "data        " << t0+fgbOffset << "   " << lgb << endl;
      fout << "t0          " << t0 << endl;
    } else if (fMsrData->getT0Tag() == T0_FROM_MUSR_T0) {
      fout << "data        120  " << lgb << endl;
      fout << "t0          100  " << endl;
    }
    if (i<noOfDetec-1)
      fout << "#-----------------------------------------------" << endl;
  ...
  fout << "range  0.0  " << fMsrData->getFitEnd() << " -0.35 0.35" << endl;
...
int PMusrWiz::writeMsrFileAsymmetry()
{
  ...
  if (setup->getNoOfLogicalAsymDetectors() > 1)
    fout << "runs   1-" << setup->getNoOfLogicalAsymDetectors() << endl;
  else
    fout << "runs   1" << endl;
  fout << "range  0.0  " << fMsrData->getFitEnd() << " -0.35 0.35" << endl;
  fout << "view_packing " << fMsrData->getPacking() << endl;
